...
def standardize_gff(input_file, output_file, species_prefix, tool_name, genome_dir=None):
    ...
    with open(input_file, 'r') as f:
        for line_num, line in enumerate(f, 1):
            ...
            if start > end:
                start, end = end, start
                fixes.add(f"Swapped coordinates for features")
            ...
            if chrom_lengths and seqid in chrom_lengths:
                ...
                if start > chrom_len or end > chrom_len:
                    fixes.add(f"Skipped features exceeding chromosome length ({seqid}: {chrom_len:,} bp)")
                    continue
    ...
    for o in other_features:
        ...
        if not parent_id or parent_id not in feature_map:
            fixes.add(f"Dropped orphan {o['type']} features")
            continue
        ...
        if parent['type'] == 'mRNA':
            ...
            new_mrna = {
                ...
                'strand': o['strand'],
                'phase': '.',
                'attributes': {'ID': new_mrna_id, 'Parent': parent_id},
                'children': [o],
            }
